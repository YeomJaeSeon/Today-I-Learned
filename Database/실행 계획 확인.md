...
- 드라이빙 - 드리븐 테이블 결정의 조건은 다양하지만 일반적으론 레코드 개수가 적은 테이블이 드라이빙 테이블.
...
```
![explain_extra_null_key_2.png](images%2Fexplain_extra_null_key_2.png)
...
- using where + filtered 칼럼의 값을 이용해 쿼리 성능을 확인할수 있다.
